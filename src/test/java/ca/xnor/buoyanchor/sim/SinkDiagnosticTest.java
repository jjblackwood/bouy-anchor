    ...
    void traceTdl030_in_5kt() {
        ...
        for (int s = 0; s < 60 * 240; s++) {
            ...
            if (next < cps.length && t >= cps[next]) {
                System.out.printf("t=%5.1fs  tilt=%+6.1f°  y=%6.2f  Fwind=%5.2f N  Fline=(%+6.1f, %+6.1f)  Fb=%6.1f%n",
                    t, tilt, sim.buoy.y, sim.buoy.Fwind,
                    sim.rode.topConstraintFx, sim.rode.topConstraintFy, sim.buoy.FbuoyancyTotal);
                next++;
            }
        }
