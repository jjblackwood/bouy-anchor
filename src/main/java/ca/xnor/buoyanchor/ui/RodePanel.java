...
/**
 * Edit the ordered list of rode segments (anchor → buoy) as a table. # kind gauge length(m) kg/m
 * MDF diam(mm) × For chain/rope rows the gauge dropdown drives kg/m, MDF, and diameter (those
 * fields show the looked-up values, disabled). For OTHER rows the gauge dropdown is disabled and
 * the user edits kg/m, MDF, and diameter directly.
 */
public class RodePanel extends VBox {
  ...
  public RodePanel(SimParams params) {
    ...
    ComboBox<RodePreset> presetChooser =
        new ComboBox<>(FXCollections.observableArrayList(RodePreset.LIBRARY));
  ...
  private void rebuild() {
    ...
    for (int i = 0; i < params.segments.size(); i++) {
      ...
      ComboBox<RodeSegment.Kind> kindCombo =
          new ComboBox<>(FXCollections.observableArrayList(RodeSegment.Kind.values()));
      ...
      gaugeCombo.setMinWidth(W_GAUGE);
      gaugeCombo.setPrefWidth(W_GAUGE);
      ...
      TextField diam =
          numField(seg.diameterM.get() * 1000.0, W_DIAM, v -> seg.diameterM.set(v / 1000.0));
  ...
  static TextField numField(
      double init, double prefWidth, java.util.function.DoubleConsumer onChange) {
    TextField tf = new TextField(formatPlain(init));
    tf.setMinWidth(prefWidth);
