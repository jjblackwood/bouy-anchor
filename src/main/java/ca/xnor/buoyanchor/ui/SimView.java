...
/**
 * Resizable Canvas + AnimationTimer. Runs physics substeps and redraws each frame on the JavaFX
 * Application Thread.
 */
public class SimView extends Pane {

  private static final System.Logger LOG = System.getLogger(SimView.class.getName());
  ...
  public void setReadout(ReadoutPanel r) {
    this.readout = r;
  }
        ...
        new AnimationTimer() {
          ...
          public void handle(long now) {
            try {
              ...
              // JavaFX silently swallows AnimationTimer exceptions, which can leave the canvas
              // frozen with nothing visibly wrong. Surface the failure on stderr so it's obvious.
              // Errors (OOM, AssertionError) are intentionally NOT caught — those should kill the
              // app rather than be papered over.
              LOG.log(System.Logger.Level.ERROR, "SimView frame failed", ex);
            }
          }
        };
    timer.start();
  ...
  private void updateTransform(double w, double h) {
    ...
    // Horizontal world span we want visible (anchor at ~10% from left, rest of the field on the
    // right).
  ...
  private double sx(double xWorld) {
    return originXpx + xWorld * pxPerM;
  }

  private double sy(double yWorld) {
    return surfaceYpx - yWorld * pxPerM;
  }

  /**
   * Local water surface elevation in world metres at world x, accounting for waves. Uses
   * sim.wavePhase (continuously evolving) so changes to Tp don't visually warp the waves.
   */
  private double surfaceElevation(double xWorld) {
  ...
  private void draw() {
    ...
    for (int i = 0; i <= samples; i++) {
      ...
      double yWorld = surfaceElevation(xWorld);
      xs[i] = xPx;
      ys[i] = sy(yWorld);
    }
    xs[samples + 1] = w;
    ys[samples + 1] = seabedYpx;
    xs[samples + 2] = 0;
    ys[samples + 2] = seabedYpx;
    ...
    if (r.n > 1) {
      for (int i = 0; i < r.n - 1; i++) {
        ca.xnor.buoyanchor.model.RodeSegment.Kind k =
            r.kind[i + 1] != null ? r.kind[i + 1] : r.kind[i];
        switch (k) {
          case CHAIN -> {
            g.setStroke(Color.web("#2e2e2e"));
            g.setLineWidth(3.5);
          }
          case ROPE -> {
            g.setStroke(Color.web("#c9a877"));
            g.setLineWidth(2.0);
          }
          case OTHER -> {
            g.setStroke(Color.web("#5a8a5a"));
            g.setLineWidth(2.5);
          }
          // Null kind on a fresh node (or any future kind we haven't drawn yet) falls back to
          // chain-coloured but thinner — better than a NullPointerException in the renderer.
          case null, default -> {
            g.setStroke(Color.web("#2e2e2e"));
            g.setLineWidth(2.0);
          }
   ...
   * buoy bottom (line tension), buoy top (wind drag). Magnitudes are physically meaningful; arrow
   * lengths are scaled so a typical 100 N renders ~50 px.
   */
  private void drawForces(GraphicsContext g, Rode r) {
    ...
    drawArrow(
        g,
        sim.buoy.attachWorldX(),
        sim.buoy.attachWorldY(),
    ...
    // 3) Anchor — chain pull on the anchor (= tension at node 0, pointing from anchor toward node
    // 1).
    if (r.n > 1) {
    ...
    for (int i : marks) {
      String col =
          switch (r.kind[i] == null ? ca.xnor.buoyanchor.model.RodeSegment.Kind.CHAIN : r.kind[i]) {
            case CHAIN -> "#2e2e2e";
            case ROPE -> "#8a5a25";
            case OTHER -> "#3a6a3a";
          };
  ...
  private void drawTensionAt(
      GraphicsContext g, Rode r, int i, double Tv, double Th, double pxPerN, String colourHex) {
  ...
  /**
   * Render the anchor body at world (ax, ay). The mooring attach point sits exactly at (ax, ay) on
   * the seabed; the body of the anchor extends a short way below the bed and takes a shape
   * characteristic of the chosen anchor kind.
   */
  private void drawAnchor(GraphicsContext g, double axW, double ayW, boolean overloaded) {
    ...
    switch (k) {
      case PYRAMID -> {
        ...
        g.fillPolygon(
            new double[] {ax - baseHalf, ax + baseHalf, ax}, new double[] {ay, ay, ay + depth}, 3);
      }
      case BLOCK -> {
   ...
   * Pulled-upward = anchorPullFy exceeds the anchor's submerged weight; dragging = the horizontal
   * pull exceeds mass × g × holdingFactor. The thresholds match the figures the Analysis report
   * uses.
   */
