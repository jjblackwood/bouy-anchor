...
/**
 * Right-side input panel. Inputs are bound bidirectionally to the SimParams
 * model so edits propagate live to the running sim. Wrapped in a ScrollPane
 * because the rode-segment editor can grow tall.
 */
public class ControlPanel extends ScrollPane {
    ...
    public ControlPanel(SimParams p) {
        ...
        ComboBox<BuoyPreset> markChooser = new ComboBox<>(
                FXCollections.observableArrayList(BuoyPreset.LIBRARY));
        markChooser.setValue(p.buoy.get());
        markChooser.valueProperty().addListener((o, a, b) -> { if (b != null) p.buoy.set(b); });
        ...
        row = addRow(env, row, "Bottom μ",   p.bottomMu,  0, 2, 0.05);
        ...
        box.getChildren().addAll(
                new Label("Mark"), markChooser,
                new Separator(),
                new Label("Environment"), env,
                new Separator(),
                new AnchorPanel(p),
                new Separator(),
                new RodePanel(p),
    ...
    private static int addRow(GridPane g, int row, String label,
                              DoubleProperty prop,
        ...
        spinner.getValueFactory().valueProperty().addListener(
                (obs, oldV, newV) -> { if (newV != null) prop.set(newV); });
