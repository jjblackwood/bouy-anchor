...
public class ReadoutPanel extends HBox {
  ...
  public ReadoutPanel(SimParams params, Simulation sim) {
    ...
    for (Label l :
        new Label[] {
          markLine, massLine, cgLine, windLine, depthLine, hsLine, orbLine,
    ...
    tip(
        cgLine,
        "Centre of gravity above the keel, and the number of axial segments the buoy is sliced into.");
    tip(
        windLine,
        "Instantaneous wind (kt) — the buoy actually sees this — then the slewed mean and the user setpoint it's slewing toward.");
    tip(
        depthLine,
        "Water depth and effective fetch (the upwind distance over which waves develop).");
    tip(
        hsLine,
        "Hs = significant wave height (mean of the highest 1/3 of waves, m).\n"
            + "Tp = peak wave period (period of the dominant spectral component, s).");
    tip(
        orbLine,
        "u_orb = surface orbital velocity amplitude (m/s) of water particles under the waves.\n"
    ...
    tip(
        nodesLine,
        "Total rode length, number of Verlet/PBD nodes the rode is discretised into, plus anchor.");
    tip(
        buoyLine,
        "drift = horizontal distance of the buoy keel from the anchor (m, +x downwind).\n"
    ...
    tip(
        fwaveLine,
        "Net wave-related force (Morison inertial + wave-orbital drag) on the buoy (N).");
    tip(
        flineLine,
        "Rode reaction on the buoy: horizontal Fx, vertical Fy, and total tension |T| (N).");
  ...
  private static VBox column(String header, Label... lines) {
    ...
    setHgrow(col, Priority.SOMETIMES);
    col.setMinWidth(USE_PREF_SIZE);
    return col;
  }
  ...
  public void refresh() {
    ...
    massLine.setText(
        String.format(
            "%5.1f kg dry, %5.1f kg ballast @%3.0f cm",
            preset.dryMassKg, preset.ballastMassKg, preset.ballastHeightAboveKeelM * 100));
    cgLine.setText(
        String.format("CG %3.0f cm,  %2d segs", bu.cgAboveKeelM * 100, bu.segments.size()));

    double instantKt = sim.actualWindKnots;
    double meanKt = sim.meanWindKnots;
    windLine.setText(
        String.format(
            "wind  %4.1f kt  (mean %4.1f → %4.1f)", instantKt, meanKt, params.windKnots.get()));
    depthLine.setText(
        String.format("depth %5.2f m   fetch %4.0f km", params.depthM.get(), params.fetchKm.get()));
    hsLine.setText(String.format("Hs    %5.2f m   Tp    %5.2f s", waves.hs, waves.tp));
    orbLine.setText(
        String.format("u_orb %5.2f m/s   d_dsgn %5.2f m", waves.uOrb, waves.designDepth));
    ...
    for (int i = 0; i < params.segments.size(); i++) {
      ...
      seg.append(
          String.format(
              "%s %.1fm", s.kind.get().name().toLowerCase(java.util.Locale.ROOT), s.lengthM.get()));
      if (i < params.segments.size() - 1) seg.append(" + ");
    }
    rodeLine.setText(seg.toString());
    nodesLine.setText(
        String.format(
            "total %5.1f m,  %3d nodes   anchor %s %5.1f kg",
            params.totalRodeLengthM(),
            r.n,
            params.anchor.get().kind.get(),
            params.anchor.get().massKg.get()));
