...
/**
 * Top-level simulation orchestrator. Tight coupling: the buoy occupies the top rode node, so the
 * rode's PBD distance-constraint solver handles force transfer between buoy and line.
 *
 * <p>Per step: 1. Recompute waves and re-discretize rode if the segment list changed. 2. Compute
 * buoy non-tension forces and torques (gravity, buoyancy, wind/wave drag). 3. Push buoy mass and
 * net acceleration to the top rode node. 4. Step the rode (Verlet + PBD). Buoy translation falls
 * out of this. 5. Read buoy position back from top node. 6. Take rode's constraint reaction (= line
 * tension on buoy) and combine its torque with the non-tension torque. Integrate buoy rotation.
 */
public final class Simulation {
  ...
  public double actualHs;
  public double actualTp = 1.0;
  public double actualUOrb;
  ...
  /**
   * Time constant for wave field to approach equilibrium after a wind change. Real seas take 10–30
   * min for full development; we use 90 s as a usable compromise that's long enough to look right
   * but fast enough to see during a session.
   */
  public static final double WAVE_DEVELOPMENT_TAU_S = 90.0;

  private double[] lastSegLengths;
  private double lastDepth = -1;
  private BuoyPreset lastPreset;
  ...
  public void step(SimParams p, double dt) {
    ...
    // (Re)configure buoy if the preset changed. Compare by name rather than identity so a
    // round-trip through StateStore (which may rehydrate a fresh BuoyPreset instance) still
    // counts as the same preset.
    if (lastPreset == null || !preset.displayName.equals(lastPreset.displayName)) {
    ...
    double waveOrbAccel =
        (waves.tp > 0.01) ? waves.uOrb * (2 * Math.PI / waves.tp) * Math.cos(wavePhase) : 0;

    buoy.computeEffectiveMassAndInertia(localSurface, p.waterDensity.get(), depth);

    Buoy.Forces nonTension =
        buoy.computeNonTensionForcesAndTorque(
            localSurface,
            p.waterDensity.get(),
            p.airDensity.get(),
            waves.u10,
    ...
    rode.setBuoyNode(
        buoy.effectiveMass, nonTension.Fx / buoy.effectiveMass, nonTension.Fy / buoy.effectiveMass);
