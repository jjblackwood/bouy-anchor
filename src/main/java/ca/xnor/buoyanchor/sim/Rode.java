 ...
 * inverse-mass weighting, seabed contact, anchor pin at node 0, and the buoy occupying the top
 * node (its mass is set externally).
 *
 * Coordinates: world, y-up, metres. Anchor at (anchorX, anchorY).
 *
 * Node 0 = anchor end. Node n-1 = buoy. Segments are laid down anchor → buoy.
 */
public final class Rode {

    public static final double TARGET_LINK_M = 0.10;
    /** PBD iterations per step with bidirectional Gauss-Seidel sweeps so info propagates both
     *  ways each iteration. 64 was empirically sufficient — the trace at 200 was identical, so
     *  the system converges well before that. Most rode links are slack and skipped, so this is
     *  cheaper than the worst case. */
    ...
    public double seabedY = 0;
    public double bottomMu = 0.6;
    public double linDamping = 0.999;       // tiny numerical floor; real damping is hydrodynamic now

    /** Constraint reaction recorded at the top node after the last step.
     *  This is the force the rode exerts on the buoy (Newtons, world frame). */
    public double topConstraintFx = 0, topConstraintFy = 0;

    /** Tension the bottom rode link applies to the anchor pin (Newtons, world frame).
     *  +Fy = pulled upward (lifting); large |Fx| = pulled sideways (drag).
     *  Derived from the cumulative PBD corrections that link 0-1 applies to node 1 across all
     *  iterations: the impulse needed to keep node 1 at rest length from the (infinite-mass)
     *  anchor pin is the reaction the pin feeds back into the chain. */
    public double anchorPullFx = 0, anchorPullFy = 0;

    /** Allocate / re-discretize for the given ordered segments (anchor → buoy). */
    public void rebuild(List<RodeSegment> segments,
                        double initialBuoyAttachX, double initialBuoyAttachY) {
    ...
    /** Recompute per-node mass, inverse-mass, and net downward acceleration for non-top nodes.
     *  Top node (buoy) is left alone — caller sets it via setBuoyNode.
     *
     *  Killicks attach at the node closest to the requested distance, measured along the rode
     *  from either the buoy or the anchor depending on each killick's {@code fromAnchor} flag. */
    public void recomputeMasses(List<RodeSegment> segments,
        ...
        for (int s = 0; s < segments.size(); s++) {
            ...
            int links = 0;
            // Recover this segment's link count from segStep we used in rebuild — but easier:
            // total node count for this segment = links + (s==0 ? 1 : 0) (start node shared).
            // Equivalently: re-derive from kind boundaries. Simpler approach — recompute links
            // the same way rebuild does, then advance nodeIdx.
            double L = Math.max(0, seg.lengthM.get());
            links = Math.max(1, (int) Math.round(L / TARGET_LINK_M));
        ...
        if (killickMassKg != null && killickMassKg.length > 0) {
            ...
            for (int k = 0; k < killickMassKg.length; k++) {
                boolean fromAnchor = killickFromAnchor != null && k < killickFromAnchor.length
                                     && killickFromAnchor[k];
                ...
                for (int i = n - 2; i >= 0; i--) {
                    ...
                    if (err < bestErr) { bestErr = err; idxK = i; }
                }
    ...
    public void step(double dt) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i != top) {
                ...
                double linkLength = (i > 0 ? linkLen[i - 1] * 0.5 : 0)
                                  + (i < n - 1 ? linkLen[i] * 0.5 : 0);
