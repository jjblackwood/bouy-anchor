 ...
 *
 * <p>Each segment carries its own mass (shell distributed by volume + ballast lump assigned to the
 * slice containing the ballast height) and is the unit of force application: buoyancy is computed
 * per-segment against the local water surface, wind drag per emerged segment, wave drag per top-Hs
 * segment. Forces are summed and torqued about the buoy's CG to drive the rigid-body rotation.
 * Translation is performed by the rode (the buoy is the top rode node, with its mass set externally
 * each step).
 *
 * <p>Sign convention: theta = 0 is upright, +theta tips the body axis from +y toward +x
 * ("downwind"). That makes +theta a CW rotation in the standard 2D frame, so the rotational
 * integrator negates the standard CCW-positive torque before applying it.
 */
public final class Buoy {
  ...
  public static final class Segment {
    ...
    public final double
        mass; // total mass of this slice (kg) — includes any ballast lump assigned here
    ...
    public double sCentroid() {
      return sBottom + length / 2.0;
    }
  ...
  /**
   * Estimated wave-radiation damping for pitch (N·m·s/rad). For a slim cylinder there's very little
   * heave radiation (axisymmetric motion radiates poorly), so we don't model it — end-cap drag is
   * the dominant heave damper.
   */
  ...
  public double
      effectiveInertia; // dry + hydrodynamic added moment of inertia, recomputed each step

  public double x, y; // keel world position (m)
  public double vx,
      vy; // translational velocity at the keel (set externally each step from rode Verlet)
  ...
  // Bookkeeping for legend / debug. Fwind/Fwave drive the live readout; FbuoyancyTotal is read
  // by SimulationFloatingTest and SinkDiagnosticTest to verify the vertical force balance.
  ...
  /**
   * Effective mass / inertia for the integrators. We use DRY values: hydrodynamic added mass is
   * implicit in the per-segment drag forces (which include the body's resistance to accelerating
   * through water). Adding it to the inertia AS WELL would double-count and tends to put the
   * natural pitch period right on top of the wave period for typical spar buoys, causing unphysical
   * resonance. Drag through water is what really damps the motion.
   */
  public void computeEffectiveMassAndInertia(
      double localSurfaceY, double waterDensity, double depthM) {
    effectiveMass = mass;
  ...
  /**
   * Slice each preset section into ~10 cm chunks. Shell mass is distributed proportionally to slice
   * volume; the ballast lump is added to the slice that contains ballastHeightAboveKeelM.
   */
  private static List<Segment> buildSegments(BuoyPreset p) {
    ...
    if (p.ballastMassKg > 0 && !segs.isEmpty()) {
      ...
      for (int i = 0; i < segs.size(); i++) {
        ...
        if (bH >= s.sBottom && bH <= s.sBottom + s.length) {
          idx = i;
          break;
        }
      }
  ...
  public double attachWorldX() {
    return x + preset.attachAboveKeelM * Math.sin(theta);
  }

  public double attachWorldY() {
    return y + preset.attachAboveKeelM * Math.cos(theta);
  }

  public double cgWorldX() {
    return x + cgAboveKeelM * Math.sin(theta);
  }

  public double cgWorldY() {
    return y + cgAboveKeelM * Math.cos(theta);
  }

  public Forces computeNonTensionForcesAndTorque(
      double localSurfaceY,
      double waterDensity,
      double airDensity,
      double windU10,
      double waveOrbVel,
    ...
    for (Segment seg : segments) {
      ...
      // plus tangential velocity from rotation about CG: v_rot = omega_my × (sc - cgH)
      // (axis-tangent)
      double rArm = sc - cgAboveKeelM; // along body axis, signed
      // Tangential direction (perpendicular to body axis, in the rotation direction): for +omega_my
      // (CW)
      ...
      if (subLen > 1e-6) {
        ...
        if (relSpeed > 1e-5) {
          ...
          // Only credit wave drag toward Fwave_acc when there's actually orbital motion to drag
          // against — otherwise this is just hull viscous drag against the buoy's own motion.
          Fwave_acc += (waterVx != 0) ? Fdx : 0;
        }
      }
      ...
      if (cosT >= 0) {
        emergStart = sub[1];
        emergEnd = seg.sBottom + seg.length;
      } else {
        emergStart = seg.sBottom;
        emergEnd = sub[0];
      }
   ...
   */
  static double[] submergedAxisRange(
      double sBottom, double h, double keelY, double surfaceY, double cosT) {
  ...
  /**
   * tauStd in standard CCW-positive convention. Our theta is CW-positive, so we negate. No
   * artificial angular damping — rotational damping comes from real fluid drag on the segments
   * (computed in computeNonTensionForcesAndTorque).
   */
