...
/**
 * One ordered section of the anchoring rode (between anchor and buoy). The rode is built as a list
 * of these — anchor end first, buoy end last.
 *
 * <p>For chain and rope, a {@code gauge} string identifies a standard size in {@link #CHAIN_GAUGES}
 * / {@link #ROPE_GAUGES} and the corresponding linear density, line diameter, and MDF are looked up
 * from the table. For {@code OTHER} segments the gauge is empty and the user supplies the per-meter
 * mass, MDF, and diameter directly.
 */
public final class RodeSegment {
  ...
  public static final List<Gauge> CHAIN_GAUGES =
      List.of(
  ...
  public static final List<Gauge> ROPE_GAUGES =
      List.of(
  ...
  /** No-arg constructor — all fields take their declared defaults. */
  public RodeSegment() {
    // Defaults come from the field initialisers above.
  }
  ...
  public static RodeSegment ofGauge(Kind kind, String gaugeLabel, double lengthM) {
    ...
    if (g == null)
      throw new IllegalArgumentException("Unknown gauge '" + gaugeLabel + "' for " + kind);
    RodeSegment s = new RodeSegment();
    s.kind.set(kind);
    s.gauge.set(gaugeLabel);
    s.name.set(kind.name().toLowerCase(Locale.ROOT));
  ...
  /**
   * Legacy constructor — assumes OTHER-style explicit specs (kind is honoured but no gauge lookup
   * happens). Kept for tests and for loading old saved state.
   */
  public RodeSegment(Kind kind, double lengthM, double kgPerM, double mdf) {
    this(kind, kind.name().toLowerCase(Locale.ROOT), lengthM, kgPerM, mdf);
  }
  ...
  /**
   * Apply a gauge to this segment, overwriting kg/m, MDF, and diameter from the table. Sets kind to
   * match if needed.
   */
