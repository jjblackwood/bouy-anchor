...
/**
 * Live, observable simulation parameters. UI controls bind to these; the physics step reads them at
 * the top of each substep.
 *
 * <p>The rode is described as an ordered list of {@link RodeSegment}s from the anchor end (index 0)
 * to the buoy end (last). Killicks may be attached at any distance from the buoy along the full
 * rode. The anchor is its own object carrying mass, MDF, and holding factor.
 */
public class SimParams {

  public final ObjectProperty<BuoyPreset> buoy = new SimpleObjectProperty<>(BuoyPreset.TDL_025M_S);

  public final DoubleProperty windKnots = new SimpleDoubleProperty(15.0);

  /**
   * Autocorrelation time (s) for wind fluctuations — controls how "twitchy" the gusts feel and the
   * window over which the actual wind averages back to the setpoint. ~20–40 s is typical for marine
   * boundary layer; 30 s is a reasonable default.
   */
  public final DoubleProperty gustTauS = new SimpleDoubleProperty(30.0);

  /**
   * Wind variability as a fraction of the mean (1σ). Real marine BL turbulence intensity is ~10–20
   * %; 0.20 keeps gust factor (peak / mean ≈ 1 + 2.5σ/μ) at a credible ~1.5.
   */
  ...
  public final ObservableList<RodeSegment> segments =
  ...
  public final ObservableList<KillickSpec> killicks =
      FXCollections.observableArrayList(
