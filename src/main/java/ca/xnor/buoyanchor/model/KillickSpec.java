 ...
 * When {@code fromAnchor} is false the distance is measured from the buoy down the rode; when true
 * it's measured from the anchor up the rode. The simulation realises the killick as extra mass on
 * the nearest rode node — it may land on any segment kind (chain, rope, or other). Each killick
 * carries its own MDF.
 */
